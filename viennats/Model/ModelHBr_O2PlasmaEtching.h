...
///HBr/O2 plasma etching model
	...
	class HBr_O2PlasmaEtching {
		...
		static const double A_Si_Br;
		double A_O;
		static const double A_p;
		static const double A_SiO2;

		static const double Eth_Si_Br;
...
//        int stacked_mat;
		...
		HBr_O2PlasmaEtching(const std::string & Parameters) {//, int stacked_material) {
...
//		    stacked_mat=stacked_material;
		...
		void CalculateVelocity(double &Velocity, const VecType& NormalVector, const double *Coverages, const double *Rates, int Material, bool Connected, bool Visible) const {
			...
			if (Material==0) {	//Si
				Velocity=(-(k_Sigma_SiBr*Coverages[1]/x_Br+Rates[0]+Rates[1]*Coverages[1])/rho_Si);	//in cm/s      //TODO
			} else {
				Velocity=0.;//(-Rates[5]*Coverages[1]/rho_SiO2);										//in cm/s     //TODO
        ...
        template <class PT, class NormVecType> void ParticleCollision(
			...
			switch(p.Type) {
				case 0: {	//ION
					...
					const double Y_Si=A_Si_Br*std::max(sqrtE-std::sqrt(Eth_Si_Br),0.)*f_Si_phi;
				...
				case 2: {		//FLUORINE
					const double StickingProbability=std::max(Gamma_Br*(1.-Coverages[0]-Coverages[1]),0.);//(1.-Coverages[1]),0.);
...
//                            double dot // dot product between the incoming particle direction and the normal vector
	...
	double const HBr_O2PlasmaEtching::rho_Si=5e22; 		//in atoms/cm^3
	double const HBr_O2PlasmaEtching::rho_SiO2=2.6e22; 	//in atoms/cm^3

	double const HBr_O2PlasmaEtching::beta_Sigma_Si=4e13;	//in cm^-2*s^-1
	double const HBr_O2PlasmaEtching::k_Sigma_SiBr=1.;

	double const HBr_O2PlasmaEtching::x_Br=3.;

	double const HBr_O2PlasmaEtching::MinEnergy=1.;	//TODO //in eV
	double const HBr_O2PlasmaEtching::end_probability=0.001;

	double const HBr_O2PlasmaEtching::Gamma_O=1.;
	double const HBr_O2PlasmaEtching::Gamma_Br=0.1;			//Bromine addition

	double const HBr_O2PlasmaEtching::Eref_max=1.;
	double const HBr_O2PlasmaEtching::Phi_inflect=1.55334303;
	double const HBr_O2PlasmaEtching::n_r=1.;
	double const HBr_O2PlasmaEtching::n_l=10.;

	double const HBr_O2PlasmaEtching::A_Si_Br=5.;
	double const HBr_O2PlasmaEtching::A_p=0.0337;
	double const HBr_O2PlasmaEtching::A_SiO2=0.2;

	double const HBr_O2PlasmaEtching::Eth_Si_Br=30.;
	double const HBr_O2PlasmaEtching::Eth_O=10.;
	double const HBr_O2PlasmaEtching::Eth_p=20.;
	double const HBr_O2PlasmaEtching::Eth_SiO2=8.;

	double const HBr_O2PlasmaEtching::ion_exponent=1000.;

	double const HBr_O2PlasmaEtching::Phi_min=1.3962634;

//	const double HBr_O2PlasmaEtching::Phi_min;

}

#endif /*MODELHBr_O2PLASMAETCHING_H_*/
